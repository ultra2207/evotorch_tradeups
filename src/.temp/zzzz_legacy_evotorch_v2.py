...
class CustomMutation(Operator):
    ...
    def _mutate_individual(self, individual):
        ...
        # Determine the number of items to replace for both collections based on mutation rate
        ...
        if base_num_to_replace > 0:
            base_new_items = self._generate_unique_items(base_num_to_replace, current_price_set, current_float_set, self.base_case)
            base_positions_to_replace = random.sample(range(self.base_collection_count), base_num_to_replace)
            base_positions_to_replace.sort()

            for idx, pos in enumerate(base_positions_to_replace):
                mutated[pos] = base_new_items[idx]['price']
                mutated[pos + num_items] = base_new_items[idx]['floatvalue']

        if modifier_num_to_replace > 0:
            modifier_new_items = self._generate_unique_items(modifier_num_to_replace, current_price_set, current_float_set, self.modifier_case)
            modifier_positions_to_replace = random.sample(range(self.base_collection_count, self.base_collection_count + self.modifier_collection_count), modifier_num_to_replace)
            modifier_positions_to_replace.sort()

            for idx, pos in enumerate(modifier_positions_to_replace):
                mutated[pos] = modifier_new_items[idx]['price']
                mutated[pos + num_items] = modifier_new_items[idx]['floatvalue']

        # Check for unique values in the range TOTAL_NUM_ITEMS to TOTAL_NUM_ITEMS-1
        repeated_values = check_unique_values(mutated)
        if repeated_values:
            for value, indices in repeated_values.items():
                for index in indices:
                    if index-TOTAL_NUM_ITEMS < self.base_collection_count:
                        replacement_item = self._generate_unique_items(1, current_price_set, current_float_set, self.base_case)[0]
                    else:
                        replacement_item = self._generate_unique_items(1, current_price_set, current_float_set, self.modifier_case)[0]
    ...
    def _generate_unique_items(self, num_items, current_price_set, current_float_set, case):
        available_items = [
            item for item in self.case_items[case]
...
class CustomCrossOver(CrossOver):
    ...
    def _do_cross_over(self, parents1: torch.Tensor, parents2: torch.Tensor) -> SolutionBatch:
        ...
        for i in range(NUM_PARENTS):
            ...
            if len(unique_base_items) > self.base_collection_count:
...
def main(start_index, end_index):
    ...
    for i, (tradeup_key, tradeup_data) in enumerate(tqdm(tradeup_items, desc=f'Processing Tradeups {start_index}-{end_index}')):
        ...
        profiler.add_function(ga.step)
        profiler.add_function(CustomCrossOver._do_cross_over)
        profiler.add_function(CustomMutation._do)
        profiler.add_function(evaluate)
